...
const homestaySchema = z
  .object({
    ...
    previewDescriptionVi: z.string().trim().max(500).optional().default(""),
    description: z.string().trim().min(10).max(10000),
    descriptionVi: z.string().trim().max(10000).optional().default(""),
    ...
    locationLabelVi: z.string().trim().max(200).optional().default(""),
    seasonalRatesNote: z.union([z.string().trim().max(10000), z.null()]),
    seasonalRatesNoteVi: z.string().trim().max(10000).optional().default("")
  })
  .refine(
    (body) =>
